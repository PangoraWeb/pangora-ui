...
import { compactNumber } from '@/app/util/numbers'
...
const TrendingCommunity = ({
  ...
  slug,
  colors,
}: TrendingCommunityArguments) => {
  return (
    /*<Button className="bg-transparent p-2 hover:bg-gray-900" fullWidth>
            <div className="flex justify-between">
                <div className="flex gap-5">
                    <div className={`bg-white rounded-full p-0.5 bg-gradient-to-br ${colors}`}>
                        <Avatar size="md" src={icon} className={`bg-black`} />
                    </div>
                    <div className="flex flex-col gap-1 items-start justify-center">
                        <h4 className="text-small font-semibold leading-none text-default-600">{name}</h4>
                        <h5 className="text-small tracking-tight text-default-400">{slug}</h5>
                    </div>
                </div>
                <div className="text-slate-500 flex items-center">
                    <div className="flex px-2">
                        ...
                        <p className="ml-2">{compactNumber(users)}</p>
                    </div>
                    <div className="flex px-2">
                        ...
                        <p className="ml-2">{compactNumber(posts)}</p>
                    </div>
                    <div className="flex px-2">
                        ...
                        <p className="ml-2">{compactNumber(comments)}</p>
                    </div>
                </div>
            </div>
        </Button>*/
    <Link
      as={NextLink}
      className="p-2 flex justify-between rounded-lg"
      href={`/c/${slug}`}
    >
      <div className="flex gap-5">
        <div
          className={`bg-white rounded-full p-0.5 bg-gradient-to-br ${colors}`}
        >
          <Avatar size="md" src={icon} className={`bg-black`} />
        </div>
        <div className="flex flex-col gap-1 items-start justify-center">
    ...
    /*<Link >
            <Card className="bg-transparent p-2 hover:bg-gray-900" isPressable>
                <div className="flex justify-between">
                    <div className="flex gap-5">
                        <div className={`bg-white rounded-full p-0.5 bg-gradient-to-br ${colors}`}>
                            <Avatar size="md" src={icon} className={`bg-black`} />
